		...
		<div className="bg-[#31C4B9]/10 rounded-lg p-8 text-center w-full max-w-2xl">
			<div className="flex flex-col md:flex-row justify-between items-center space-y-8 md:space-y-0 md:space-x-8">
				<div className="flex-1 w-full">
					...
					<p className="text-4xl md:text-5xl lg:text-6xl font-bold text-white">${cash.toLocaleString()}</p>
				</div>
				<div className="flex-1 w-full">
					<h2 className="text-[#31C4B9] text-xl md:text-2xl font-semibold mb-3">Transactions Protected</h2>
					<p className="text-4xl md:text-5xl lg:text-6xl font-bold text-white">{transactions.toLocaleString()}</p>
				</div>
			</div>
		</div>
	)
