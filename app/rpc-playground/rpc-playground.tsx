					...
					{selectedMethod.params.map((param) => (
						<div key={param}>
							...
							<Input
								...
								onChange={(e) => handleParamChange(selectedMethod.params.indexOf(param), e.target.value)}
								placeholder={`Enter ${param}`}
								className="bg-white/5 border-white/10 text-white"
							/>
						</div>
					))}
					<Button type="submit" disabled={loading} className="bg-[#31C4B9] hover:bg-[#31C4B9]/90 text-black w-full">
